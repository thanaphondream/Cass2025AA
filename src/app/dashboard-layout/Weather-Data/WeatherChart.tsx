...
const CustomTooltip = ({ active, payload, viewMode }: TooltipProps<ValueType, NameType> & { viewMode: ViewMode }) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="p-3 bg-white border border-gray-300 shadow-xl rounded-lg text-sm transition duration-150 ease-in-out">
        ...
        <p className="text-red-600">อุณหภูมิ: <span className="font-semibold">{formatValue(data.temperaturde, '°C')}</span></p>
        <p className="text-blue-600">ความชื้น: <span className="font-semibold">{formatValue(data.humidity, '%')}</span></p>
        <p className="text-green-700">ฝน (ชั่วโมง): <span className="font-semibold">{formatValue(data.rain, 'มม.')}</span></p>
        <p className="text-purple-700">ความเร็วลม: <span className="font-semibold">{formatValue(data.windspeed10m, 'm/s')}</span></p>
        <p className="text-orange-700">ความกดอากาศ: <span className="font-semibold">{formatValue(data.slp, 'hPa')}</span></p>
      </div>
    );
  }
...
const WeatherChart: React.FC<WeatherChartProps> = ({ filteredData, viewMode, selectedVariable, getVariableDetails }) => {
      ...
      <div style={{ width: '100%', height: 400 }}>
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <XAxis
              dataKey="id" 
              tickFormatter={(value, index) => formatXAxis(filteredData[index], viewMode)}
              height={60}
            ...
            <Line 
              yAxisId="main" 
              type="monotone" 
              dataKey={selectedVariable as string} 
              stroke={variableDetails.color} 
              name={variableDetails.label}
              dot={false}
              strokeWidth={3}
              activeDot={{ r: 5, fill: variableDetails.color, stroke: variableDetails.color, strokeWidth: 2 }}
            />
