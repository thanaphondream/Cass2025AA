...
const DownloadPage = () => {
  ...
  const Day_data = async () => {
    try {
      if (idlocation === 0) {
        ...
        const rs = await fetch(`https://cass-api-data.vercel.app/api/climate/${year}/${month}/${day}/${pushnamelocation}`);
        const rs_json = await rs.json();
  ...
  const Month_data = async () => {
    try {
      if (idlocation === 0) {
        ...
        const year = selectedDate.getFullYear();
        const month = selectedDate.getMonth() + 1;
        const rs = await fetch(`https://cass-api-data.vercel.app/api/climate/${year}/${month}/${pushnamelocation}`);
        const rs_json = await rs.json();
  ...
  const Year_data = async () => {
    try {
      ...
      const year = selectedDate.getFullYear();
      const rs = await fetch(`https://cass-api-data.vercel.app/api/climate/${year}/${pushnamelocation}`);
      const rs_json = await rs.json();
    ...
    <div className="px-4 py-10 bg-gray-50 min-h-screen flex flex-col items-center">
      <div className="w-full max-w-6xl space-y-8">
        ...
        <div className="flex flex-wrap justify-center gap-4">
          <button className="bg-blue-700 hover:bg-blue-800 text-white font-semibold py-2 px-4 rounded-md transition duration-200" onClick={Day_data}>
            ข้อมูลรายวัน
          </button>
          <button className="bg-green-700 hover:bg-green-800 text-white font-semibold py-2 px-4 rounded-md transition duration-200" onClick={Month_data}>
            ข้อมูลรายเดือน
          </button>
          <button className="bg-pink-700 hover:bg-pink-800 text-white font-semibold py-2 px-4 rounded-md transition duration-200" onClick={Year_data}>
            ข้อมูลรายปี
          </button>
        </div>
